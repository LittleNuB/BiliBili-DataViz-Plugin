...
let activeHistoryAggregateWrites = 0;
const historyAggregateIdleWaiters = new Set<() => void>();
...
export function runHistoryAggregateDataOperation(
  operation: () => Promise<void>,
): Promise<boolean> {
  if (activeHistoryDataOperation !== null) return Promise.resolve(false);

  activeHistoryAggregateWrites++;
  return Promise.resolve()
    .then(operation)
    .then(() => true)
    .finally(() => {
      activeHistoryAggregateWrites--;
      if (activeHistoryAggregateWrites === 0) {
        for (const resolve of historyAggregateIdleWaiters) resolve();
        historyAggregateIdleWaiters.clear();
      }
    });
}
...
function runExclusiveHistoryDataOperation<T>(
  ...
  if (activeHistoryDataOperation !== null
    || (kind === 'sync' && activeHistoryAggregateWrites > 0)) {
  ...
  return Promise.resolve()
    .then(async () => {
      if (kind === 'clear') {
        await Promise.all([
          waitForHistoryPlayerEventWrites(),
          waitForHistoryAggregateWrites(),
        ]);
      }
      return operation();
    })
...
function waitForHistoryAggregateWrites(): Promise<void> {
  if (activeHistoryAggregateWrites === 0) return Promise.resolve();
  return new Promise(resolve => historyAggregateIdleWaiters.add(resolve));
}

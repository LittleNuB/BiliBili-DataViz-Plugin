...
每个 Windows 浏览器阶段都由版本固定的原生 launcher 创建一个独立、未命名的 Job Object。launcher 源码和包含 `/noconfig`、`/nostdlib+`、warning-as-error 在内的固定编译参数属于基准源码绑定，环境中的 `csc.rsp` 和隐式框架引用不能改变构建；运行时用固定系统 C# 编译器及显式 `mscorlib.dll`、`System.dll` 在全新的临时目录编译一次，环境收据记录编译器 SHA-256 和两份引用的带 framing 聚合 SHA-256。编译结果会立即哈希，并在每次启动前连同源码、编译器和引用重新校验，任何缓存复用、字节变化、缺失或空文件都失败关闭。

launcher 先创建带 `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` 且不允许 breakaway 的 Job，再通过 `PROC_THREAD_ATTRIBUTE_JOB_LIST` 在同一次 `CreateProcess` 中以 suspended 状态创建 Chrome。Chrome 一旦创建成功便已经属于 Job，不存在“创建后、分配前”的崩溃窗口；首线程只在创建成功后恢复。另一份显式 handle allowlist 只继承标准句柄和两根 CDP pipe，不开放 TCP 端口。Chrome 后续创建的子孙进程都继承 Job 成员资格，即使某个中间进程在任何轮询前就启动孙进程并退出，也不能逃离完整进程树边界；本门禁不再以进程表轮询或 PID lineage 推断作为完整性证据。创建或恢复失败时，launcher 必须先观察 Job 清空，再返回失败。

阶段结束时保持 CDP 管道打开，Node 只向 launcher 发送一次 `terminate`。launcher 在首次活动成员 precheck 前启动唯一、包含终点的绝对 5 秒窗口，precheck 与 `TerminateJobObject` 原生调用耗时都占用同一窗口；只有在 5 秒内（恰好 5 秒仍包含）读到 `ActiveProcesses == 0`，才能输出精确的 `terminated` 回执并以 0 退出，5 秒之后返回的空读表不能通过。已经为空的 Job 不能把 Chrome 提前退出改判成正常终止。Node 从 spawn 到 exit 连续监听控制 stdout 并丢弃式消费 Chrome stderr，另设有界等待，超时或失败会强制回收 controller 并继续确认其退出；只有 Job 清空、严格回执和 controller 成功退出全部成立，才关闭 CDP 并清理 profile 与生产副本。launcher 意外退出时，最后一个 Job handle 关闭会触发 kill-on-close 兜底，但缺少正常回执的运行仍永久失败，不能因兜底清理成功改判。

编译、启动、握手、句柄继承、Job 分配、线程恢复、控制写入、Job 终止、清空观察、controller 退出或强制回收任一失败都失败关闭。控制协议只接受固定 ready、terminate 和 terminated 三个状态，额外/超长控制 stdout、未知命令或非零退出均拒绝。Chrome stderr 不属于控制协议，也不会提升进收据。failure marker 只记录封闭浏览器阶段码，不保存协议正文、编译器或可执行文件路径、PID、命令输出、stderr 或 raw error。已证明的主错误在后续清理也失败时保持优先。
...
每次夹具准备、单轮运行、夹具清理和最终报告安装前，runner 都会在检查点目录中安装唯一 `active-run.json`。单轮只有在 lifecycle、checkpoint 校验/安装、profile 清理全部完成后才删除 lease；删除后才把该轮计为完成。任一异常会尝试以不覆盖方式安装 `failure.json`，并保留 active lease。failure marker 只含 session/环境绑定摘要、公开夹具与候选身份、封闭阶段/分类、可选的受控 browser/harness code 和已完成 checkpoint 数；不写异常正文、stack、stderr、URL、路径、PID 或凭据。harness code 只能由浏览器内合成 harness 的 factory 创建，并经模块私有 `WeakMap` 来源登记、固定 envelope 和字符集校验后传出；Node 浏览器控制链只能从封闭 stage code 集合中创建外层 code，并由另一个模块私有 `WeakMap` 登记。浏览器控制码进一步区分生产副本、环境、进程、扩展加载、worker、合成启动响应、harness 执行、观测收敛、Job 终止和 CDP 清理；compiler、launcher、controller、Job 与进程错误只能归入封闭外层码，不读取或提升其正文、stack、cause、stderr、退出状态、协议输出或路径。已证明的主错误在清理失败时保持优先。两侧伪造同名自有属性都不会通过来源校验。CDP exception description、`Error.message`、stack 与 cause 都不会被解析或提升为 marker 字段。这里的安装原子性仅指同目录 Node 可观察边界，不宣称断电持久性。
...
环境和每项操作通过 SHA-256 精确绑定；验证器会重算全部收据并拒绝字段、覆盖、哈希或规范序列化漂移。验证器还会重新哈希当前基准源码、完整生产源码输入、`package-lock.json`、`dist` 和全部 A1 golden receipts，确认记录的基准提交仍为当前 HEAD 的祖先；一组内部自洽但已经过期的报告不能通过。生成时记录的 Windows Job launcher 编译器与引用 identity 属于生成环境证据，并由环境收据及全部 operation receipts 固定；便携验证不要求验证主机重新安装同一历史系统编译器。CI 在干净 Windows checkout 上重新构建并运行完整 artifact verifier。四份报告先全部写入同目录临时文件，再作为一个 bundle 安装；Node 进程可观察到的任一安装失败会回滚整组旧产物。这里不宣称四个独立文件具备跨进程崩溃或断电原子性；若发生这类中断，验证器会对缺失或混合版本的产物失败关闭，随后须从严格绑定的检查点重新生成并安装完整 bundle。

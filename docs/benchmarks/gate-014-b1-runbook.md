...
5 秒窗口结束时未确认 lineage 归零，本轮即永久失败。只有父进程已经确认退出且 lineage 仍非空时，runner 才会继续使用同一份初始进程表和内存中已观察到的后代身份，在 `(5s, 10s]` 与 `(10s, 15s]` 两个固定窗口做诊断观察；不会再次执行 `taskkill`、重试工作负载、释放 failure lease 或把晚归零改判为通过。failure marker 只区分“第一诊断窗确认归零”“第二诊断窗确认归零”“15 秒前未确认归零”“诊断观察不可用”，不保存 PID、精确归零时间、轮询次数、命令输出或错误正文。只有多次独立失败都在第一诊断窗确认归零，才可另行评审是否把门槛改为 10 秒，并且必须从零重跑 canary 与完整矩阵；第二窗、15 秒仍未归零或诊断不可用都转入 Job Object / Chrome 生命周期专项，不放宽本门禁。

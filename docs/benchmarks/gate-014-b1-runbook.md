...
每次浏览器阶段结束时，runner 必须终止整个 Chrome 进程树并观察到父进程退出；仅发出 kill、只结束父进程或等待超时都不算成功。在 Windows 上，runner 保持 CDP 管道存活，先读取进程 ID 与父进程 ID，并确认根进程仍在终止前进程表中且尚未退出；原生 `taskkill /T /F` 必须成功启动并完成。原生命令返回后启动一个绝对 5 秒的共享收敛窗口，父进程退出观察和已捕获 lineage 的 50 ms 轮询归零会立即并发开始，并用 `allSettled` 等待两者都收敛；不会在父进程退出后重新获得另一段 5 秒。若两者都失败，父进程退出失败保持原有优先级；若父进程通过，lineage 失败必须独立传播。每次轮询都以终止前同一份不可变进程表为基线，并累计已经观察到的后代身份，因此父进程先退出时，其仍存活的后续子孙不会从证据中丢失。已完成的原生终止尝试即使返回数值非零，也只有在“父进程退出”和“lineage 无残留”两个独立后置条件同时成立时才可通过；该结果不推断非零退出的具体原因。命令无法启动、超时、被 signal 中断、任一次进程表读取失败、父进程未在共享窗口内退出或窗口结束时存在残留后代仍失败关闭。只有完整闭环成立后才关闭 CDP 管道；检查不读取命令行、可执行文件路径或浏览器 profile，也不把退出码、进程 ID、轮询次数、路径或命令输出写入收据。

每个 Windows 浏览器阶段在首次终止前读表之前创建且只创建一个受管 PowerShell 进程表 observer，并等待它通过版本固定的 `gate-014-b1-process-table-observer-v1` ready 握手。observer 只以固定 `-NoLogo -NoProfile -NonInteractive -Command <constant script>` 启动，stdin/stdout 只承载逐行 JSON；请求 ID 必须严格递增，任一时刻只允许一条请求在途，响应及每个进程行都必须精确匹配封闭 schema，跨边界的数据只有 `processId` 与 `parentProcessId`。首次终止前读表、5 秒门槛内的每次读表及失败后的两个诊断窗口都复用同一实例；不会在阶段内启动替代 observer，也不会回退到逐次启动 PowerShell。超时、并发请求、未知或重复响应、额外字段、协议解析失败、任意 stderr 活动、读写失败或进程提前退出都会永久 poison 该实例并使当前运行失败关闭；迟到响应不能恢复或复用它。observer 自身按父子关系不属于 Chrome lineage，且不能替代 Chrome 根进程或后代证据。启动、正常关闭和强制回收均有界；存在未完成请求时关闭会先 poison 并终止 observer，正常关闭必须等到进程与 stdio 全部关闭并确认无半行残留。若主流程已有模块证明的受控失败，该失败在 observer 清理也失败时保持优先；没有主失败时，observer 清理失败会阻止本轮通过。stderr 只形成布尔故障，不保留正文；协议正文、PID、路径和错误文本均不进入检查点、失败标记或报告。

共享收敛窗口内每次 PowerShell 进程表读取的 timeout 都取当时剩余毫秒数，而不是另给独立的 10 秒；读表完成时已经越过绝对截止时间，即使返回空表也失败关闭。lineage 读表与父进程退出观察并发开始，父进程观察不会推迟首次读表。

5 秒窗口结束时未确认 lineage 归零，本轮即永久失败。父进程已经确认退出且 lineage 仍非空时，runner 会继续使用同一份初始进程表和内存中已观察到的后代身份，在 `(5s, 10s]` 与 `(10s, 15s]` 两个固定窗口做诊断观察；第二次诊断观察不会在绝对 10 秒边界前开始。该已知残留路径任一诊断读表失败都会立即标记诊断观察不可用，不会越过证据缺口后再用空表宣称归零。若父进程已经确认退出，但 5 秒内的 lineage 读表以模块私有 `WeakMap` 证明的“进程表命令越过绝对截止时间”失败，已经失败的运行也会使用同一份初始进程表与根身份进入这两个诊断窗口；只有这一精确受控错误可以触发该路径，父进程失败、其他受控错误或伪造同名字段都不能触发。后续空表只能标记“进程表观察在第一或第二诊断窗恢复”，不能标记 lineage 已归零；恢复后的非空表会继续观察到第二窗，届时仍非空则标记 15 秒内存在残留，第二窗无法读表则无论第一窗是不可用还是读到非空，都只标记第二诊断窗观察不可用，不声称该状态持续到绝对 15 秒。

诊断不会再次执行 `taskkill`、重试工作负载、释放 failure lease 或把晚归零、观察恢复改判为通过。failure marker 只保留封闭分类，不保存 PID、精确时间、轮询次数、命令输出或错误正文。只有多次独立失败都以 5 秒门槛内成功的 lineage 读表证明存在残留、并在第一诊断窗确认归零，才可另行评审是否把门槛改为 10 秒，并且必须从零重跑 canary 与完整矩阵；“观察恢复”类别不能作为延长门槛的证据。第二窗归零、15 秒仍有残留或诊断观察不可用都转入 Job Object / Chrome 生命周期专项，不放宽本门禁。

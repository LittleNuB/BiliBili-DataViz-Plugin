...
每次浏览器阶段结束时，runner 必须终止整个 Chrome 进程树并观察到父进程退出；仅发出 kill、只结束父进程或等待超时都不算成功。在 Windows 上，runner 保持 CDP 管道存活，先读取进程 ID 与父进程 ID，再要求原生 `taskkill /T /F` 成功，并在父进程退出后再次读取同类关系；任何已捕获后代或其后续子进程仍存活都会失败关闭。只有这三项证据全部成立后才关闭 CDP 管道。这个检查不读取命令行、可执行文件路径或浏览器 profile。根进程提前退出、进程身份不可用、进程关系无法读取、终止命令非零、残留后代或 5 秒内未退出均阻止后续冷/暖运行。

...
GATE-014-A1 + GATE-014-A2 + GATE-014-B1
  +--> GATE-014-C
  +--> GATE-014-D

GATE-014-B1 + GATE-014-C + GATE-014-D
  --> GATE-014-E
         |
         +--> DB-014-A
                |
...
- 只有通过隐私、来源和代表性 review 后，才允许从该聚合收据派生独立的校准 fixture 配置；不得改写已经冻结并被 B1 收据引用的 GATE-014-A1 receipts。

不包含：登录态采集、Cookie/profile/login-state 读取、用户历史或账号数据、候选搜索/ZIP 实现。

验收：提交可复核且不含原文/账号标识的聚合收据，并把高碎片化样本绑定到最大实测尾部；校准收据、派生配置和 fixture 收据缺少上述可机器复核绑定时不得通过；找不到合法且有代表性的来源时保持 `insufficient_evidence`，不得启动 GATE-014-C/D 或提出真实工作负载代表性结论，但不阻塞只验证合成工程边界的 GATE-014-B1。

### GATE-014-B1：500 MiB IndexedDB 与恢复暂存合成工程基准
...
- 基于 GATE-014-A1 的确定性普通与高碎片化 fixture，选出写入/读取批次、进度反馈阈值和 restore staging 的合成基线配额预留系数；浏览器配额估算只可提前拒绝，不能授权最终提交。
- 按 Gate 合同记录总耗时、批次 median/p95/maximum、首次/最大进度间隔、重启状态可见时间、取消后写入、主线程阻塞和可测 heap；严格执行 15 分钟写入/暂存、10 分钟读取/修复/清理、2 秒批次 p95/进度间隔、5 秒批次 maximum/重启可见等合成数值门槛。
- 结果只证明已声明合成负载下的容量、原子性、恢复和响应性，不得写成真实 B 站字幕分布、真实用户 p95 或平台级性能结论。
- GATE-014-A2 后续通过后，可在 GATE-014-E 前对性能参数做一次校准确认；校准可提高保守参数，但不推翻 B1 已证明的工程正确性。

验收：全部 required receipt 字段和数值门槛具有明确 pass/fail/`insufficient_evidence`，并按“最大记录数、再最大字节数”的固定顺序选择通过候选；输出原始收据，明确将代表性与最大实测尾部标为 `insufficient_evidence`，不改产品 UI，也不关闭 GATE-014-A2。

### GATE-014-C：全局词法检索候选基准

依赖：GATE-014-A1、GATE-014-A2、GATE-014-B1。
...
依赖：GATE-014-B1、GATE-014-C、GATE-014-D。
...
1. 合并 DOC-014。
2. GATE-014-A1 通过后即可派发 GATE-014-B1；GATE-014-A2 独立继续取证。只有 A2 与 B1 都通过后才并行推进 C/D，最后由 E 裁决并冻结参数。
...
| GATE-014-A2 | #262 | GATE-014-B1 | #238 |

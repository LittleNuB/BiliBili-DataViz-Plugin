# [归档] Bili-Bill 动态账单开发方案

> 本文记录早期实施设想，保留用于追溯，不代表当前实现。其中将全局历史保留期改为 180 天的方案未交付；现行动态账单与数据口径以 [0.13 发布合同](./PRD-0.13-release-contract.md) 和 [动态账单合同](./PRD-dynamic-bill.md) 为准。

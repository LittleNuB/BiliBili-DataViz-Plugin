...
B1 repeat evidence is session-fail-closed rather than retry-selective. Only a deliberate checkpoint-bound pause may resume. Each setup, run, cleanup, and report-finalization phase is guarded by an exclusive active lease; an exception or interrupted lease permanently invalidates that session for both execution and verification, and requires deleting every checkpoint before restarting from zero. A later diagnostic pass does not supersede an earlier failed attempt. The retained latch is public-safe and bounded to binding digests, public fixture/candidate identity, closed failure classification, and an optional controlled harness code; it excludes raw errors, stacks, stderr, URLs, paths, process identifiers, and credentials.

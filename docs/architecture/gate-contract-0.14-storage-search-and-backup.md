...
Every B1 operation receipt records fixture ID, batch candidate, operation kind, total duration, committed batch count, committed-batch duration median/p95/maximum, separately named read-batch count/median/p95/maximum, separate all-instrumented-batch diagnostics, first progress-event latency, maximum progress-event gap, restart-state visibility latency, cancellation acknowledgement, post-cancellation writes, main-thread maximum, and peak measurable heap growth. Scanning reads and subsequent writes are timed separately. Every operation records at least its final ledger/visible-version readback; admission and restore additionally record the pre-commit invisibility and post-commit visibility scans. Only successfully committed write transactions enter the committed-batch thresholds; read-only and deliberately aborted transactions remain diagnostic. The committed count must equal the committed-duration evidence length. Every classified read and committed duration, including duplicate values, must appear in the all-instrumented diagnostics with at least the same multiplicity; count-only consistency is insufficient. Any operation whose contract includes a committed write must provide at least one sample in that class, and all 13 operations must provide at least one read sample; zero samples or any missing required measurement is `insufficient_evidence`. The final report additionally contains all 1,170 candidate/fixture/operation/cold-or-warm groups with measured-run counts and total-duration plus read and committed-write median/p95 summaries.

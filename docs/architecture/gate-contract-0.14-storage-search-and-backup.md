...
- Run as an unpacked production build in the exact official Chrome for Testing Stable version bound to the retained pre-run last-known-good metadata on Windows, using a fresh temporary test profile with no Bilibili login, Cookie import, browser-profile read, user database, API key, or runtime network dependency. Stable channel, revision, timestamp, Win64 download URL, metadata SHA-256, an exact `Google Chrome for Testing` product name, and an exact four-part executable version must all match. Prefix, suffix, rebranded, or version-drifted executables fail closed. Every browser stage must terminate the complete Chrome process tree and observe process exit before the next stage; an unavailable process identity, failed tree termination, or exit timeout fails the run.
- The matrix requires a clean tracked worktree, rebuilds `dist` itself, and independently hashes complete production source inputs and the resulting unpacked build. Record OS build, CPU, logical cores, memory, free disk, actual and declared Stable Chrome versions, extension commit, Node version, package lock hash, candidate versions, fixture hashes, storage estimate, run start/end time, and whether memory metrics were available.
- Chrome proxy use is disabled and external DNS is mapped to `~NOTFOUND` for the whole Chrome process, so the benchmark has no external runtime network dependency even if the loaded production extension attempts its normal Bilibili API startup request. In the fresh temporary profile only, the public-safe harness uses its test-only management permission to require exactly one enabled harness identity and exactly one enabled production identity matching the rebuilt manifest, then polls for at most the 30-second setup window and revalidates only the already loaded production service-worker target through `chrome.runtime`; a product page, background page, or shared worker cannot substitute for this proof. No extra product page is opened and the measured workload is unchanged. After DevTools attach, CDP observation covers every loaded or newly appearing extension page, background page, service worker, and shared worker; every launch must record at least one production-extension target and one harness target. External HTTP(S) request attempts are counted, while any observed external response or extension-attributable Runtime, console, or `chrome-extension://` log error fails the run. Error-level CDP `Log` entries without an extension source are counted separately without retaining their text and do not masquerade as extension failures. Persisted receipts retain scope and aggregate counts only, never the enumerated extension inventory or IDs. Pre-attach startup events cannot be recovered and must be explicitly recorded as unobserved, so zero observed responses are not described as a complete startup trace.

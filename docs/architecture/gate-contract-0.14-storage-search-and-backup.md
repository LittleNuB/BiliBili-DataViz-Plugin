...
- External DNS is blocked for the whole Chrome process, so the benchmark has no external runtime network dependency even if the loaded production extension attempts its normal Bilibili API startup request. After DevTools attach, CDP observation covers every loaded or newly appearing extension page, background page, service worker, and shared worker; external HTTP(S) request attempts are counted, while any observed external response or console error fails the run. Persisted receipts retain scope and counts only. Pre-attach startup events cannot be recovered and must be explicitly recorded as unobserved, so zero observed responses are not described as a complete startup trace.

...
- Run as an unpacked production build in the exact official Chrome for Testing Stable version bound to the retained pre-run last-known-good metadata on Windows, using a fresh temporary test profile with no Bilibili login, Cookie import, browser-profile read, user database, API key, or runtime network dependency. Stable channel, revision, timestamp, Win64 download URL, metadata SHA-256, an exact `Google Chrome for Testing` product name, and an exact four-part executable version must all match. Prefix, suffix, rebranded, or version-drifted executables fail closed. Every browser stage must terminate the complete Chrome process tree and observe process exit before the next stage. On Windows, the runner keeps the CDP pipe open while native `taskkill /T /F` runs, requires the root to be present and still running before termination, and requires the native attempt to start and complete. It then observes the parent exit, snapshots process-ID and parent-process-ID relationships again, and rejects any captured descendant or descendant spawned from that lineage that remains after the root exits. A completed numeric-nonzero native outcome is accepted only when these independent parent-exit and zero-survivor postconditions pass; the outcome does not claim why the command returned nonzero. The numeric exit code, process IDs, paths, and command output are not retained in receipts. A root that exits before the explicit termination step, an unavailable process identity, unavailable process-tree observation, command spawn failure, command timeout or signal interruption, surviving descendant, or parent-exit timeout fails the run.

...
动态账单（动态版块）面向「兴趣再平衡」而非推荐排序：它先帮你识别你可能长期关注但近期关注度下降或漏看的 UP/内容，再决定是否值得打开，不做“猜你喜欢”式的实时点击预测。
...
安装版本约定：当前发布分支与扩展清单对外显示版本均为 `0.9.0-alpha`（Manifest 内置 `version: 0.9.0`，通过 `version_name: 0.9.0-alpha` 暴露预发布标识）。
...
- Bili-Bill 不会上传完整历史记录、完整关注列表、Cookie、用户 mid、个人资料或 feedback 记录到任何第三方。

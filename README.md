...
0.12.0-alpha 当前范围包括 Bili-Bill 内容账单、动态账单、智能收藏、AI 设置与本地数据隐私入口、当前视频助手、智能收藏问答、视频知识节点 v0 与视频盲盒候选实验。

动态账单（动态版块）面向「兴趣再平衡」：它先帮你识别你可能长期关注但近期关注度下降或漏看的 UP/内容，再决定是否值得打开。
...
安装版本约定：当前 npm/package 元数据版本为 `0.12.0-alpha`；Chrome MV3 Manifest 内置 `version: 0.12.0`，通过 `version_name: 0.12.0-alpha` 暴露预发布标识。

...
2. **接受扩展权限**：扩展需要访问 `*.bilibili.com`，用于读取登录态 Cookie 并调用 B站 API。
3. **等待首次同步**：安装后，扩展会自动同步你最近的观看历史（最多约 1000 条）。如果安装时尚未登录，点击 Popup 的刷新按钮会重新触发首次回填。
4. **继续正常使用 B站**：扩展每 5 分钟自动增量同步你的最新观看记录。数据会持续累积在本地。

### 同步与统计口径

- **登录态检测**：Service Worker 通过 `www.bilibili.com`、`api.bilibili.com`、`passport.bilibili.com` 的 `SESSDATA` Cookie 判断是否已登录，并在请求 API 时使用浏览器登录态。
- **首次回填**：未完成回填或本地历史为空时，手动刷新会强制重新拉取可用历史，避免“已登录但无数据”的状态卡住。
- **增量同步**：回填完成后，每 5 分钟拉取最近一页历史并按 `kid` 与 `[avid+cid+viewAt]` 去重。
- **聚合补算**：同步完成后会按本地已保存历史逐日重算聚合，Dashboard 使用聚合数据展示。
- **自然周/月**：本周统计为周一到今天；本月统计为自然月 1 号到今天。例如今天是 5 月 14 日，本月数据范围就是 5 月 1 日到 5 月 14 日。
- **本地时区**：日期分桶使用浏览器本地时间，避免凌晨数据被算到 UTC 前一天。
...
A: 先确认已经在同一个浏览器 Profile 中登录 B站。如果已登录仍提示未登录，请到 `chrome://extensions` 重新加载扩展；如果刚更新过代码或权限，也需要接受新的 `*.bilibili.com` 权限后再点 Popup 刷新。

### Q: Dashboard 图表显示空白？

A: 可能是尚未同步到数据。确认已经在 B站 观看过视频，点击 Popup 刷新按钮触发同步；首次回填后扩展会自动重算历史聚合。如果仍为空，打开 `chrome://extensions` → Service Worker DevTools，查看 `[BiliViz]` 日志和 IndexedDB 的 `watchHistory` / `dailyAggregates` 表。

### Q: 本周/本月统计范围是什么？

A: 本周按自然周计算，即周一到今天；本月按自然月计算，即当月 1 号到今天，不是最近 7 天或最近 30 天。

### Q: 更新代码后为什么还是旧行为？

A: 修改 `public/manifest.json` 或重新构建后，需要在 `chrome://extensions` 中点击扩展卡片的刷新按钮。只刷新 B站 网页不会重新加载 Service Worker 和 manifest 权限。

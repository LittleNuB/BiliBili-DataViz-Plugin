...
- 点击“同步收藏夹”会读取当前 B站登录账号的收藏夹列表和收藏视频，保存到本地 IndexedDB。
- 在“AI 配置”中填写 OpenAI 兼容接口参数，默认适配 DeepSeek：`https://api.deepseek.com` 和 `deepseek-v4-flash`。
- 点击“生成智能索引”会批量生成分类路径、摘要、关键词和可检索别名；当前默认每批最多处理 200 条。
- 顶部统计会区分收藏视频、索引成功、索引失败和待索引；失败项再次生成索引会重试。
...
- 智能收藏夹 AI 能力依赖用户提供的 OpenAI 兼容 API Key，模型请求失败时会记录为索引失败并允许重试。

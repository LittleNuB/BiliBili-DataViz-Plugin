...
function lifecycleResult() {
  return {
    ...
    operations: B1_OPERATION_KINDS.map(operation => ({
      operation,
      expectedDirection: ["admission", "restore_staging"].includes(operation) ? "increase" : "stable",
      ...
      storageAfter: {
        usageBytes: ["admission", "restore_staging"].includes(operation) ? 2_000 : 1_000,
        quotaBytes: 10_000,
      },
      readbackVerified: true,
...
test("GATE-014-B1 lifecycle mapping fails closed when browser storage metrics are absent", () => {
  ...
  assert.equal(createB1OperationReceipt(rawOperation).status, "insufficient_evidence");
});
